...
fluxion_startup() {
  ...
  local requiredCLITools=(
    ...
    "php-cgi" "tmux" "rfkill" "unzip" "route:net-tools"
...
if ! tmux -2 -c "airodump-ng -Mat WPA "${2:+"--channel $2"}" "${3:+"--band $3"}" -w \"$FLUXIONWorkspacePath/dump\" $1" 2> $FLUXIONOutputDevice; then
	echo -e "$FLUXIONVLine$CRed $FLUXIONGeneralXTermFailureError"
    sleep 5
    return 2
fi    
...
fluxion_target_tracker_daemon() {
  ...
  Old_AP_Authenticator_Pid=$(ps a | grep -E "captive_portal_authenticator" | head -n 1 | awk '{print $1}')
